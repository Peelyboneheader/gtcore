...
class _PlannerApp:
    ...
    def _refresh_shadowing(self):
        ...
        tiles, 0.4 s at eight -- so it is skipped in three cases: mid-drag
        (that path budgets tens of milliseconds), above ``SHADOW_MAX_TILES``,
        and during startup adoption of the fitted implant, where the latency
        would buy an answer the user has not asked for yet.  The first real
        edit runs it.  Any failure silently means "nothing to report": this
        is advisory, and it must never take the planner down.
        """
        pairs = []
        if (2 <= len(self.tiles) <= SHADOW_MAX_TILES
                and self._drag_idx < 0 and not self._adopting):
            try:
    ...
    def _adopt_fitted_tiles(self):
        ...
        if adopted:
            self.selected = 0
            # Skip the shadowing sweep for THIS refresh only. It costs up to
            # half a second on a big implant, and at startup the user has
            # not asked a question yet -- paying that in launch latency buys
            # an answer nobody requested. The first real edit runs it.
            self._adopting = True
            try:
                self._after_change(
